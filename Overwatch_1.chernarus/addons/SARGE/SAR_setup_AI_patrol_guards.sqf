...
//_authorizedUID = _flagPole getVariable ["AuthorizedUID", []];
...
_leader_weapon_names = ["leader",_type] call SAR_Guard_unit_loadout_weapons;
...
_tVariations = if (random 1 >= 0.5) then { true; } else { false; };
...
for "_i" from 0 to (_snipers - 1) do
{
    ...
    _sniper_weapon_names = ["sniper",_type] call SAR_Guard_unit_loadout_weapons;
...
for "_i" from 0 to (_riflemen - 1) do
{
    ...
    _soldier_weapon_names = ["rifleman",_type] call SAR_Guard_unit_loadout_weapons;
