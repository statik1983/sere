...
if(isPlayer _aikiller) then {
    ...
    if (_aikilled_group_side == SAR_AI_friendly_side) then { // hit a friendly AI
        ...
        if(SAR_HITKILL_DEBUG)then{diag_log format["SAR_HITKILL_DEBUG: friendly AI was hit by Player %1",_aikiller];};
        ...
        if(SAR_HITKILL_DEBUG)then{diag_log format["SAR_HITKILL_DEBUG: Adjusting humanity for survivor hit by %2 for %1",_aikiller,(SAR_surv_kill_value/10)];};
        
        if((rating _aikiller > -10000)) then { //check if shooter is not already marked as enemy
            if(SAR_HITKILL_DEBUG)then{diag_log format["SAR_HITKILL_DEBUG: Marking Player %1 as an enemy for a friendly AI hit!",_aikiller];};
    ...
    if (_aikilled_group_side == SAR_AI_unfriendly_side) then { // hit an unfriendly AI
        ...
        if(SAR_HITKILL_DEBUG)then{diag_log format["SAR_HITKILL_DEBUG: unfriendly AI was hit by Player %1",_aikiller];};

        _humanity = _aikiller getVariable ["humanity",0];
        _humanity = _humanity + (SAR_band_kill_value/10);
        _aikiller setVariable["humanity", _humanity,true];
        
        if(SAR_HITKILL_DEBUG)then{diag_log format["SAR_HITKILL_DEBUG: Adjusting humanity for bandit hit by %2 for %1",_aikiller,(SAR_band_kill_value/10)];};
