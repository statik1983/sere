...
for "_i" from 0 to ((count allbuildables) - 1) do
	{
		...
		_result = [typeOf(_obj),_classname] call BIS_fnc_areEqual;
			if (_result) exitwith {
				...
				_qtyC = _recipe select 7;
				_qtyP = _recipe select 8;
				_qtyU = _recipe select 9;
				_qtyA = _recipe select 10;
				_qtyZ = _recipe select 11;
				_qtyI = _recipe select 12;
				_qtyJ = _recipe select 13;
				_qtyN = _recipe select 14;
				_qtyQ = _recipe select 15;
				_qtyR = _recipe select 16;
				_qtyB = _recipe select 17;
				_qtyX = _recipe select 18;				
			};
	};
	if (_qtyT > 0) then {
	...
	if (_qtyP > 0) then {
		for "_i" from 1 to _qtyP do { _result = [player,"equip_wood_pallet"] call BIS_fnc_invAdd;  };
	};	
	if (_qtyU > 0) then {
		for "_i" from 1 to _qtyU do { _result = [player,"equip_duct_tape"] call BIS_fnc_invAdd;  };
	};		
	if (_qtyA > 0) then {
		for "_i" from 1 to _qtyA do { _result = [player,"ItemCamoNet"] call BIS_fnc_invAdd;  };
	};	
	if (_qtyZ > 0) then {
		for "_i" from 1 to _qtyZ do { _result = [player,"PartGlass"] call BIS_fnc_invAdd;  };
	};
	if (_qtyI > 0) then {
		for "_i" from 1 to _qtyI do { _result = [player,"equip_nails"] call BIS_fnc_invAdd;  };
	};
	if (_qtyJ > 0) then {
		for "_i" from 1 to _qtyJ do { _result = [player,"ItemJerrycan"] call BIS_fnc_invAdd;  };
	};
	if (_qtyN > 0) then {
		for "_i" from 1 to _qtyN do { _result = [player,"ItemGenerator"] call BIS_fnc_invAdd;  };
	};
	if (_qtyQ > 0) then {
		for "_i" from 1 to _qtyQ do { _result = [player,"equip_metal_sheet"] call BIS_fnc_invAdd;  };
	};
	if (_qtyR > 0) then {
		for "_i" from 1 to _qtyR do { _result = [player,"equip_rope"] call BIS_fnc_invAdd;  };
	};
	if (_qtyB > 0) then {
		for "_i" from 1 to _qtyB do { _result = [player,"equip_brick"] call BIS_fnc_invAdd;  };
	};
	if (_qtyX > 0) then {
		for "_i" from 1 to _qtyX do { _result = [player,"ItemTent"] call BIS_fnc_invAdd;  };
	};
