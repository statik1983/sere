		...
		{	
			if ((_x getVariable ["characterID", "0"]) != "0") then {
				...
				if (!procBuild && _isBomb == 1 && !((getPlayerUID player) in _authorizedPUID)) then {
					...
					if (!isNull _bomb && ((player distance _bomb < 5 && (speed player > 4 || speed player < -3)) || (_vehiclePlayer distance _bomb < 6 && (_inVehicle && speed _vehiclePlayer > 0 || _inVehicle && speed _vehiclePlayer < -0)))) then {
						...
						_blowUp = [_bomb, _pos, _vehiclePlayer] spawn {
							...
							sleep .1;
							if ((player distance _bomb < 2 && (speed player > 4 || speed player < -3)) || (_vehiclePlayer distance _bomb < 3 && (_inVehicle && speed _vehiclePlayer > 0 || _inVehicle && speed _vehiclePlayer < -0))) then {
								_detonate = "grenade" createVehicle position _bomb; /*createVehicle ["GrenadeHandTimedWest_DZ", _pos, [], 0, "CAN_COLLIDE"];*/
							} else {
								sleep .7;
								_detonate = "grenade" createVehicle position _bomb; /*createVehicle ["GrenadeHandTimedWest_DZ", _pos, [], 0, "CAN_COLLIDE"];*/
							};
