...
*/
private["_authorizedUID","_allFlags","_newAttachCoords","_startingPos","_buildables","_flagradius","_okToBuild","_allowedExtendedMode","_flagNearest","_flagNearby","_requireFlag","_funcExitScript","_playerCombat","_isSimulated","_isDestructable","_townRange","_longWloop","_medWloop","_smallWloop","_inTown","_inProgress","_modDir","_startPos","_tObjectPos","_buildable","_chosenRecipe","_cnt","_cntLoop","_dialog","_buildCheck","_isInCombat","_playerCombat","_check_town","_eTool","_toolBox","_town_pos","_town_name","_closestTown","_roadAllowed","_toolsNeeded","_inBuilding","_attachCoords","_requirements","_result","_alreadyBuilt","_uidDir","_p1","_p2","_uid","_worldspace","_panelNearest2","_staticObj","_onRoad","_cntLoop","_finished","_checkComplete","_objectTemp","_locationPlayer","_object","_id","_isOk","_text","_mags","_hasEtool","_canDo","_hasToolbox","_inVehicle","_isWater","_onLadder","_building","_medWait","_longWait","_location","_isOk","_dir","_classname","_item","_authorizedPUID","_canUseFlag","_itemL","_itemM","_itemG","_qtyL","_qtyM","_qtyG","_itemT","_itemS","_itemW","_qtyT","_qtyS","_qtyW","_itemE","_itemC","_itemP","_itemU","_itemA","_itemZ","_itemI","_itemJ","_itemN","_itemQ","_itemR","_itemB","_qtyE","_qtyC","_qtyP","_qtyU","_qtyA","_qtyZ","_qtyI","_qtyJ","_qtyN","_qtyQ","_qtyR","_qtyB","_itemX","_qtyX"];
...
_playerCombat 	= player;
	...
	// Global variables for loop method, procBuild may not be needed if implemented in fn_selfactions.sqf
		if (dayz_combat == 1) then {
			cutText ["You're currently in combat, time reduced to 3 seconds. \nCancelling/escaping will set you back into combat", "PLAIN DOWN"];
			sleep 3;
	...
	*/
			...
			if (typeOf(_x) == BBTypeOfZShield) then {
				...
				if ((getPlayerUID player) in _authorizedPUID && (_classname == BBTypeOfZShield)) then {
					...
					if (_shieldCount >= BBMaxZShields) then {
						cutText [format["Your playerUID is already registered to %1 zombie shield generators, you can only be added on up to %1 zombie shield generators.\nBuild cancelled for %2",BBMaxZShields,_text], "PLAIN DOWN"];
						sleep 1;
						call _funcExitScript;
					};
				};
	...
	if (_classname != "Grave" && _classname != BBTypeOfFlag) then {
		...
		};
		{
			...
			if (!_okToBuild && (!_requireFlag || _requireFlag) && _x distance player <= _flagRadius) then {cutText [format["Build cancelled for %1\nCannot build in other player's bases, only Booby traps are allowed.",_text], "PLAIN DOWN"];call _funcExitScript;};
			if (!_okToBuild && _requireFlag && !_flagNearby) then {cutText [format["Either no flag is within %1 meters or you have not built a flag pole and claimed your land.\nBuild cancelled for %2",_flagRadius, _text], "PLAIN DOWN"];call _funcExitScript;};
		} foreach _allFlags;
	};
...
//_inbuilding was disabled for some reason	
	if (!_inBuilding) then {
	...
	//Lets make a nice hint window to tell people the controls
		...
		//This section handles the placement where you can change position etc
			...
			if ((!(isNull _dialog) || (speed player >= 12 || speed player <= -9) || _isInCombat > 0) && (isPlayer _playerCombat) ) then {
				...
				cutText [format["Build cancelled for %1. Player moving too fast, in combat, or opened gear.",_text], "PLAIN DOWN"];hint "";call _funcExitScript;
			};
		sleep 0.03;
	};
	...
	} else {cutText [format["Build cancelled for %1. Something went wrong!",_text], "PLAIN DOWN"];hint "";call _funcExitScript;};
	...
	switch (true) do
	{
		case(_longWait):
		{
			...
			for "_i" from 0 to _longWloop do
			{
				...
				if (player distance _locationPlayer > 1) then {deletevehicle _object; cutText [format["Build cancelled for %1, position of player moved",_text], "PLAIN DOWN"];hint "";call _funcExitScript;};
				if (!_canDo || _onLadder || _inVehicle || _isWater) then {deletevehicle _object; cutText [format["Build cancelled for %1, player is unable to continue",_text], "PLAIN DOWN"];hint "";call _funcExitScript;};
				sleep 1;
				...
				if (r_interrupt) then {
					...
					cutText [format["Build cancelled for %1, position of player moved",_text], "PLAIN DOWN"];
...
//-------------	
	...
	if (_qtyU > 0 ) then {
        for "_i" from 0 to _qtyU do
        {
            player removeMagazine _itemU;
        };
    };
	if (_qtyA > 0 ) then {
        for "_i" from 0 to _qtyA do
        {
            player removeMagazine _itemA;
        };
    };
	if (_qtyZ > 0 ) then {
        for "_i" from 0 to _qtyZ do
        {
            player removeMagazine _itemZ;
        };
    };
//-------------
	if (_qtyI > 0 ) then {
        for "_i" from 0 to _qtyI do
        {
            player removeMagazine _itemI;
        };
    };
	if (_qtyJ > 0 ) then {
	...
	if (_qtyN > 0 ) then {
        for "_i" from 0 to _qtyN do
        {
            player removeMagazine _itemN;
        };
    };
	if (_qtyQ > 0 ) then {
        for "_i" from 0 to _qtyQ do
        {
            player removeMagazine _itemQ;
        };
    };	
	if (_qtyR > 0 ) then {
        for "_i" from 0 to _qtyR do
        {
            player removeMagazine _itemR;
        };
    };
	if (_qtyB > 0 ) then {
	...
	//-------------
	if (_qtyX > 0 ) then {
        for "_i" from 0 to _qtyX do
        {
            player removeMagazine _itemX;
        };
    };
