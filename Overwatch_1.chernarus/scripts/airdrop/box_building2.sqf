...
diag_log format ["Donator Building Supply box has been called in by survivor: %2 Drop GPS coordinates at: %1 Type of transport: %3",mapGridPosition _pos,dayz_playerName,_TheModel]; 
...
while {(_Drop_plane distance _wp_pos) > 500} do {
	...
	if (_hasRC) then {
		if (_DROP_Action==0) then {
			The_DropAction2 = player addAction ["<t color='#FF0000'>Drop Donator Building Supply box...</t>",_thePath+"box_building3.sqf","",5,true,true,"",""];
			_DROP_Action=1;
			hint "";
...
} else {cutText ["Dropping Donator Building Supply box", "PLAIN DOWN"];};
...
switch (true) do {
	case not (alive _Drop_plane): {
		...
		diag_log format["WARNING! The MV-22 Osprey crashed at GPS coordinates: %1, dropping supply box!\nThe supply box will be deleted in 10 minutes!",(mapGridPosition getPos _Drop_plane),_TheModel]; 
	};
	...
	case alive _Drop_plane: {
		...
		diag_log format["Donantor Building Supply box has been called in at GPS coordinates: %1",mapGridPosition getPos _Drop_plane];
...
switch (true) do {
	...
	case alive _box: {
		...
		{_bam addmagazineCargoGlobal [_x, 10];} forEach _BuildingSupply10List;	
		{_bam addmagazineCargoGlobal [_x, 5];} forEach _BuildingSupply5List;			
...
//		{_bam addmagazineCargoGlobal [_x, 1];} forEach _HALVm1List;
		...
		_txt3 = (gettext (configFile >> 'CfgVehicles' >> _randombox >> 'displayName'));
		diag_log format["Donantor Building Supply box landed at GPS coordinates: %1 smoke signal deployed", _pos3];
		if (_Del_Box) then {
...
//			hint "The box is now gone...";
			diag_log format["Donantor Building Supply box deleted at GPS coordinates: %1", _pos3];
		} else {
			_GPS = (mapGridPosition _pos3);
//			cutText [format["Donantor Building Supply box landed at GPS coordinates: %1",_GPS],"PLAIN DOWN",5];
			sleep 60;
			deletevehicle _Drop_plane;
			deletevehicle _pilot;
		};
